...
import { Plus, Search, User, ChevronRight, Trash2, AlertTriangle, CheckSquare, Square, Printer, ArrowLeft, RefreshCw } from 'lucide-react';
...
const ClientList: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    try {
        ...
        // Note: Balance calculation is still sync (local ledger) so it's fast
  ...
  const BulkLedgerSheet = ({ client }: { client: Client }) => {
      ...
      // Use a consistent default padding for bulk print
      return (
    ...
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        ...
        <div className="flex space-x-3">
             ...
             {/* Only show Seed button if list is empty to avoid duplicates easily */}
             {clients.length === 0 && (
                 <button 
                    onClick={handleSeed}

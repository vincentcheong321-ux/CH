  ...
  const renderWinningContent = (description: string | undefined, totalAmount: number, showTotal: boolean) => {
    ...
    return (
        <div className="flex flex-col w-full min-w-0 pt-0.5 overflow-visible font-mono">
            {/* Date on Top - Centered-ish above data */}
            {dateStr && (
            ...
            <div className="flex flex-col w-full min-w-0 gap-1 overflow-visible">
                {parsedLines.map((line, i) => (
                    <div key={i} className="grid grid-cols-[30px_45px_50px_35px_25px_auto] gap-1 items-center text-[11px] md:text-[14px] text-gray-800 leading-none py-0.5 w-full whitespace-nowrap">
                        ...
                        <span className="text-gray-400 text-center text-[9px] md:text-[11px] font-bold">
                            {line.big}-{line.small}
                        </span>
                        <span className="text-gray-400 text-[9px] md:text-[10px] uppercase font-bold text-center truncate">{line.type}</span>
                        
                        {/* Position Badge */}
                        <div className="flex justify-center">
                            {line.pos && (
                                <span className="px-1 py-0.5 rounded border border-gray-600 bg-white text-[10px] md:text-[12px] font-black text-gray-900 leading-none shadow-sm min-w-[20px] text-center">
                                    {line.pos}
                                </span>
                            )}
                        </div>
                        ...
                        <div className="flex items-center justify-end">
                            <span className="text-gray-300 px-1 font-light mr-1">-</span>
            ...
            {/* Total Transaction Amount - Large Display below details */}
            {showTotal && (
      ...
      <div className="flex flex-col w-full px-1">
          <div className="flex flex-col space-y-0.5 w-full">
                {data.processed.map((r) => {
                    ...
                    // Show standard description for non-winning or main ledger winning
                    // For Col1 Winning, we show the special expanded view
                    const showDescription = !(isWinning && columnType === 'main');
                    
                    // Hide side amount if it's a winning record in Col1 (we show total inside the description block)
                    ...
                    <div key={r.id} className={`group flex items-start py-1 relative gap-1 md:gap-2 w-full ${!r.isVisible ? 'opacity-30 grayscale no-print' : ''}`}>
                        ...
                        <div className="flex w-full items-start relative z-10 min-h-[24px]">
                            ...
                            {showAmountColumn ? (
                                ...
                                // Placeholder if amount is hidden (handled inside renderWinningContent for col1 winners)
